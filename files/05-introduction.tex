...
\begin{enumerate}
  ...
  \item Compiler directives : For better cache performance, sometimes compilers can attempt to allocate aligned blocks of memory. This functionality is exported to the programmer as \texttt{posix\_memalign} by \textit{GCC}, \texttt{\_\_aligned\_malloc} by \textit{MSVC} and \texttt{ippMalloc} by \textit{ICC}. These allocators may leave gaps filled with garbage values which are picked up by the cache when an entire line is fetched, thus reducing the effective caching capacity and reducing utilization.
...
In Chapter \ref{chap:background}, we look more closely at the organisation and operations of a conventional cache and provide an overview of related work. Chapter \ref{chap:ac_architecture} describes the Amoeba Cache architecture. The experimental setup and implementation of the simulator are described in Chapter \ref{chap:implementation}. The experimental results of an exhaustive evaluation of the Amoeba-Cache is presented in Chapter \ref{chap:evaluation}. Conclusions and future work are outlined in Chapter \ref{chap:conclusions}.
